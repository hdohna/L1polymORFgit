...
# General description:
#
#   The following function calculates the variance in the frequency of an allele
...
VarAlleleFreq <- function(s, N = 10^4, SampleSize = 2*2504, probIns = 0.5, DetectProb, LogRegCoeff){
  ...
  ProbRef <- function(x) {
    ...
    ExpVal <- (integrate(function(x) {
      ...
      IntConst
    (integrate(function(x) {
      (x - ExpVal)^2* AlleleFreqTime(x, s, N) * (1 - ProbRef(x)) 
    }, 0, 1)$value - 
        integrate(function(x) {
          (x - ExpVal)^2 * AlleleFreqTime(x, s, N) * (1 - DetectProb * x)^SampleSize * (1 - ProbRef(x)) 
        }, 0, 1)$value) / 
      IntConst
