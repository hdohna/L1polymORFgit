...
Pops        <- SampleInfo$pop[SampleMatch]
...
for (Chr in c(1:22, "X")) {
  ...
  L1Coeffs <- sapply (idxEnough, function(i) {
    ...
    DistAfter3  <- Singletons$V2[SampleMatch3After] - Line1Vcf$V2[i]
...
blnSig <- L1SingletonCoeffs$Pr...z.. < 0.05
points(L1SingletonCoeffs$InsLength[blnSig], L1SingletonCoeffs$coef[blnSig], 
       col = "red")
L1SingletonCoeffs$InsLBins <- cut(L1SingletonCoeffs$InsLength, breaks = seq(0, 6500, 1000))
MeanCoeffPerL <- aggregate(L1SingletonCoeffs[,c("coef", "InsLength")],
                           by = list(L1SingletonCoeffs$InsLBins), FUN = mean)
