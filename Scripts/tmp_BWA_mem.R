...
which(!SC50 %in% SampleIDs)



DecodeSamBitFlag(123)
DecodeSamBitFlag(165)
DecodeSamBitFlag(133)
DecodeSamBitFlag(101)


# Bwa commands
BWACmds <- c("module load bwa",
   paste("bwa mem /labs/dflev/hzudohna/RefSeqData/hg19.fa /scratch/users/hzudohna/hg19_HG00100_sorted.fq",
   "> /scratch/users/hzudohna/hg19_HG00100.sam"))

# Sam commands to turn sam file into bam file
SamBamCmds <- c("module load samtools",
                paste("samtools view /scratch/users/hzudohna/hg19_HG00100.sam -b -h -o", 
                      "/scratch/users/hzudohna/hg19_HG00100.bam"))

# Sam commands to sort and index bam file
SamIdxCmds <- c("module load samtools",
                paste("samtools sort /scratch/users/hzudohna/hg19_HG00100.bam", 
                      "-o /scratch/users/hzudohna/hg19_HG00100_sorted_bwamem.bam"),
                paste("samtools index /scratch/users/hzudohna/hg19_HG00100_sorted_bwamem.bam"))
...
MELTCmds <- c("module load bowtie2",
              paste("java -Xmx2g -jar /labs/dflev/hzudohna/MELTv2.1.5/MELT.jar Single -bamfile",
                    "/scratch/users/hzudohna/hg19_HG00100_sorted_bwamem.bam", 
                    "-c 7",
...
AllCmds <- c(BWACmds, SamBamCmds, SamIdxCmds, MELTCmds)

# Create script name and run script
ScriptName <- "Sim_BwaMem_MELTScript"
