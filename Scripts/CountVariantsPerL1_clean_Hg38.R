...
# File paths
ResultPathCombined  <- "D:/OneDrive - American University of Beirut/L1polymORF/Data/L1VariantRegrResults_combined_HG38_2021-01-13.csv"
...
load("D:/OneDrive - American University of Beirut/L1polymORF/Data/L1HSRefRanges_hg38.RData")
L1Width <- width(L1GR)

# Load data on L1 coverage
load("D:/OneDrive - American University of Beirut/L1polymORF/Data/L1CoverageResults_HG38.RData")
...
TriNuc        <- getSeq(BSgenome.Hsapiens.UCSC.hg38, L1Cover_TriGR)
cat("done!\n")
...
L1Variants <- read.table("D:/OneDrive - American University of Beirut/L1polymORF/Data/SNPsInL1_highCover_all.vcf")
L1Variants <- L1Variants[nchar(L1Variants$ALT) == 1 & 
                           L1Variants$FILTER == "PASS",]
...
L1Var_Left  <- read.table("D:/OneDrive - American University of Beirut/L1polymORF/Data/SNPsInL1left_highCover_all.vcf")
L1Var_Right <- read.table("D:/OneDrive - American University of Beirut/L1polymORF/Data/SNPsInL1right_highCover_all.vcf")
...
L1Seq            <- getSeq(BSgenome.Hsapiens.UCSC.hg38, L1GR)
...
idxMismatchNonSyn <- lapply(NonSynPos, function(i){
  ...
  Nuc_ORF1 <- as.character(getSeq(BSgenome.Hsapiens.UCSC.hg38, GR_Pos_ORF1))
  GR_Pos_ORF2 <- GRNonSynInt[blnORF2_Nonsyn &
                               GRNonSynInt@elementMetadata@listData$mcols.ORFPos0 == i - 1]
  Nuc_ORF2 <- as.character(getSeq(BSgenome.Hsapiens.UCSC.hg38, GR_Pos_ORF2))
...
Promoters <- promoters(TxDb.Hsapiens.UCSC.hg38.knownGene, upstream = 5*10^3)
CdsTx <- cdsBy(TxDb.Hsapiens.UCSC.hg38.knownGene, "tx")
...
sum(overlapsAny(L1Cover_GR, L1VarGR))
seqnames(L1VarGR)
seqnames(L1Cover_GR)
...
SNPLogRegInt <- bigglm(blnSNP ~  TriNuc + L1VarCount_Flank + CoverMean +
                       ...
                       maxit = 20)
sum(is.na(L1CoverTable$blnSNP))
sum(is.na(L1CoverTable$TriNuc))
sum(is.na(L1CoverTable$L1VarCount_Flank))
sum(is.na(L1CoverTable$CoverMean))
sum(is.na(L1CoverTable$PropMismatch))
sum(is.na(L1CoverTable$Genes))
sum(is.na(L1CoverTable$Exons))
sum(is.na(L1CoverTable$Promoters))
sum(is.na(L1CoverTable$Dist2Edge))
sum(is.na(L1CoverTable$blnFull))
sum(is.na(L1CoverTable$Syn))
sum(is.na(L1CoverTable$Coding))

sum(L1CoverTable$blnSNP)
sum(L1CoverTable$TriNuc)
sum(L1CoverTable$L1VarCount_Flank)
sum(L1CoverTable$CoverMean)
sum(L1CoverTable$PropMismatch)
sum(L1CoverTable$Genes)
sum(L1CoverTable$Exons)
sum(L1CoverTable$Promoters)
sum(L1CoverTable$Dist2Edge)
sum(L1CoverTable$blnFull)
sum(L1CoverTable$Syn)
sum(L1CoverTable$Coding)
...
SNPLogRegInt_byORF <- bigglm(blnSNP ~  TriNuc + L1VarCount_Flank + CoverMean +
...
SNPLogReg_CodeOnlyFull <- bigglm(blnSNP ~  TriNuc + L1VarCount_Flank + CoverMean +
...
L1Aligned <- read.fasta(file = "D:/OneDrive - American University of Beirut/L1polymORF/Data/L1Hs_HG38_with_consensus_aligned.fas")

# Create a distance matrix for alignment
L1AlDist  <- dist.alignment(read.alignment(file = "D:/OneDrive - American University of Beirut/L1polymORF/Data/L1Hs_HG38_with_consensus_aligned.fas",
                                           format = "fasta"))
...
blnSameNuc <- sapply(1:1000, function(x){
  GenomeNuc <- getSeq(BSgenome.Hsapiens.UCSC.hg38, L1VarGR[SNPposDF$idxInGR[i]])[[1]]
